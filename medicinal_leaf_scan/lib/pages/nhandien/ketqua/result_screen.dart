  ...
  class _ResultScreenState extends State<ResultScreen> {
    ...
    Future<void> _initializeData() async {
      ...
      // Nếu đã đăng nhập, tự động lưu vào lịch sử
  ...
  try {
    // BỎ TOÀN BỘ PHẦN UPLOAD ẢNH
    // Chỉ lưu thông tin vào Firestore
    await FirebaseFirestore.instance.collection('history').add({
      'userId': user.uid,
      'leafName': _leafData != null
    ...
    Future<void> _saveToCollection() async {
      ...
      try {
        ...
        final fileName = '${timestamp}_${path.basename(widget.imageFile.path)}';
        final storage = FirebaseStorage.instance;
        final storageRef = storage.ref().child(
          'collections/${user.uid}/$fileName',
        );
        ...
        final uploadTask = storageRef.putFile(widget.imageFile, metadata);
    ...
    Widget build(BuildContext context) {
      ...
      final confidence = widget.recognitionResult['confidence'] != null
    ...
    Widget _buildErrorScreen() {
      return Scaffold(
        ...
        body: const Center(
          child: Text('Không có kết quả nhận dạng'),
        ),
      );
    }
